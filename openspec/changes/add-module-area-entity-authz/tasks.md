...
- [ ] 1.1 Scaffold del proyecto (estructura de carpetas de `openspec/project.md`, tsconfig, eslint, package.json, `.env.example`). Validación: `npm run typecheck` y `npm run lint` terminan en cero.
...
- [ ] 2.1 Schema de Prisma: `profiles`, `module_grants`, `grant_areas`, `entity_restrictions`, `area_closure`, y `users.profile_id`, con los campos e índices del diseño. No agregar tablas que el diseño no nombre. Validación: `prisma validate` y la migración aplican sobre Postgres limpio.
